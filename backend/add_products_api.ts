...
async function loginAsAdmin() {
    ...
    console.log('Note: You need an admin account to add products.\n');
...
async function addProducts() {
    console.log(`\n🚀 Adding ${products.length} products to ${BACKEND_URL}\n`);
    ...
    try {
        ...
        if (errorCount > 0) {
            console.log(`⚠️  Failed to add ${errorCount} products.`);
        }
